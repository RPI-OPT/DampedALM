function [out] = RobustNLLeastSquares_proxLinear(opts,x,epsilon,m,n,rho,maxiter,cons_nu, nu0)
%# Problem setting
...
%         nu = epsilon^2/rho^3;
    ...
    [x,ngrad,subiter, suberr,L] =  inner(x, A, b, B, Q, c, rho,down,up,mu,nu,L,eps_k,subit,gammau,gammad,ngrad,m,n);
    ...
    fprintf(['NumIter: %d, Obj value: %d, ' ...
        'suberr: %d, subiteration: %d, norm(x-xold): %d, Lipschitz: %d, primal err: %d\n'],i,objective,suberr,subiter,dual(i),L, norm(A*x-b));
...
function [x,ngrad,i,err,L1] = inner(x, A, b, B, Q, c, rho,down,up,mu,nu,L,eps_k,subit,gammau,gammad,ngrad,m,n)
...
p       = (Ak*x+ak*v)/(Ak + ak);
gradp   = getgrad(A,b,D,B,Q,c,xk,p,nu,m,n,rho);
T       = getT(xk,p,gradp,rho,L,down,up);
gradT   = getgrad(A,b,D,B,Q,c,xk,T,nu,m,n,rho);
...
while i < subit && err > eps_k
    ...
    gradx   = getgrad(A,b,D,B,Q,c,xk,x,nu,m,n,rho); %calculate gradient at x_k
...
%     end
    if rem(i,10) == 0
        err = getinnerviol(A,b,D,B,Q,c,xk,x,nu,m,n,rho,up,down);
    end
%     disp(err)
    i = i + 1;
end
...
function grad = getgrad(A,b,jacobianofc,B,Q,c,xk,x,v,m,n,rho)
valueofc    = zeros(2*m,1);
for i = 1:m
    valueofc(i)   = 0.5*xk'*(Q{i}*xk +2*c{i});
end
valueofc(end-m+1:end) = A*xk-b;
yk                    = valueofc + jacobianofc*(x-xk);

proxofl    = getinnerprox(yk,v,m);
grad       = (1/v)*(jacobianofc'*(yk-proxofl));
end
...
%
function innerprox = getinnerprox(y,v,m)
y1 = y(1:end-m);
% y2 = y(end-m, end);

innerprox = zeros(size(y));
innerprox(1:end-10) = sign(y1).*max(abs(y1)-v,0);
% innerprox(end-10:end) = 0;
end
...
% subproblem
function innerviol = getinnerviol(A,b,D,B,Q,c,xk,x,nu,m,n,rho,up,down)
% grad = Q0*x + c0 + A'*y + beta*A'*(A*x-b) + 2*rho*(x-xk); 
grad = getgrad(A,b,D,B,Q,c,xk,x,nu,m,n,rho) + 2*rho*(x-xk);
xi   = zeros(length(x),1);
