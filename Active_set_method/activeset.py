...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 30 11:33:44 2025
...
class ActiveSet(object):
    ...
    def _calc_objective(self, A, b, x):
...
U, _ = qr(np.random.randn(n, n))
v = np.maximum(0, np.random.randn(n) * 5)


Q = U @ np.diag(v) @ U.T + temp * np.eye(n)
Q = (Q + Q.T) / 2  # Ensure symmetry
A_top = np.random.randn(m, n - m)
A = np.hstack([A_top, np.eye(m)])

# Generate feasible x
xfeas = np.random.rand(n)
I = np.eye(n)
...
bounds = [(cl[i], cu[i]) for i in range(n)]

# Solve the problem
solution = minimize(objective, xfeas,  method='SLSQP', bounds=bounds, constraints=constraints)

# Extract solution
x_py = solution.x
fval_py = solution.fun

inst = ActiveSet()


out = inst.run(Q, c, A, b, [], [], cu, cl, xfeas)

print("fval active set " + str(out[1]) + " iteration num " + str(out[2]))
print("fval python optimizer " + str(fval_py))
