function [out] = IAPIAL_qcqp(Q0, Q, c0, c,d,down,up,sigma,opts )
...
if isfield(opts,'beta');      beta0    = opts.beta;       else,  beta0    = 2*1e-4;           end
...
while k < maxit && err > tol
   ...
   % Print the inner and outer violations, subiteration number and error
    ...
    if k > khat +1
        ...
        if deltak <= 0.5*tol1^2*lambda/Csigma && beta < 15*beta0
            beta = 2 * beta;
            khat = k;
        end
    end
