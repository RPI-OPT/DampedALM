...
rho = min(min(eig(Q{m+1})),0); % rho-wc
...
Qx = cell(1,m+1); Qxh = cell(1,m+1); % include Qxhat in case...
gradf = cell(1,m+1); gradfh = cell(1,m+1);
fval = zeros(m+1,1); fvalh = zeros(m+1,1);
psi = zeros(m,1); gradPsi = zeros(n,1); %% grad of whole AL?? 
for j = 1:m+1
    Qx{j} = Q{j}*x;
    gradf{j} = Qx{j} + c{j};
    fval(j) = .5*x'*Qx{j} + c{j}'*x + d(j);
end
%gradf{m+1} = gradf{m+1} + rho*(x-x0); %% pert
%fval(m+1) = fval(m+1) + rho*norm(x-x0)^2/2; %% pert
...
for k = 1:K0
    ...
    %gradf{m+1} = gradf{m+1} + 2*rho*(x-xk); %% pert
    %fval(m+1) = fval(m+1) + rho*norm(x-xk)^2; %% pert
    ...
    [primal, dual, compli] = getouterviol(x,Q{m+1},Q,c{m+1},c,d,z,up,down);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [x,z,beta,total_PG] = iALM_str(x0, xk, tol, rho, beta0)
        ...
        while pres > tol % only check primal_res since dual_res automatically below tol
            beta = beta*sig;
            if adp
                [x, numPG] = APG_adp(x, xk, z, rho, beta, tolk);  % bug! should be x, not x0!!
            else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function [x, numPG] = APG(x0, xk, z, rho, beta, tol) % Copied from adaptive!
    end % end of APG
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function [x, numPG] = APG_adp_old(x0, xk, z, rho, beta, tol) % old AdapAPG.
        % Our old APG_adp (good, but not theory)
        ...
        while grad_res > tol && numPG < APGmaxit
            ...
            objhat = 0.5*xhat'*Q{m+1}*xhat+c{m+1}'*xhat +rho*norm(xhat-xk)^2;
            for j = 1:m
                objhat = objhat + 0.5/beta*max(0, z(j)+beta* (0.5*xhat'*Q{j}*xhat+c{j}'*xhat+d(j)))^2;
            end
            nobj = nobj + 1;
            Lip = Lip0/dec;
            obj = inf;
            while obj > objhat + gradhat'*(x - xhat) + 0.5*Lip*norm(x - xhat)^2 + 1e-10 %& Lip < LipG
                ...
                obj = 0.5*x'*Q{m+1}*x+c{m+1}'*x +rho*norm(x-xk)^2;
                for j = 1:m 
                  obj = obj + 0.5/beta*max(0, z(j)+beta* (0.5*x'*Q{j}*x+c{j}'*x+d(j)))^2;
                end
                nobj = nobj + 1; numPG = numPG + 1;
            end
            alpha = sqrt(rho/Lip);
            extrap_weight = (1-alpha)/(1+alpha);
            %extrap_weight = alpha*(1-alpha_old)/alpha_old/(1+alpha); % original: (1 - alpha) / (1 + alpha)
            alpha_old = alpha;
            ...
            for j = 1:m
              Qx{j} = Q{j}*xhat;
              gradf{j} = Qx{j} + c{j};
              fval(j) = .5*xhat'*Qx{j} + c{j}'*xhat + d(j);
              gradhat = gradhat+ max(0,z(j)+beta*fval(j))*gradf{j};
            end
            grad = Q{m+1}*x+c{m+1} +2*rho*(x-xk); %
            for j = 1:m
              Qx{j} = Q{j}*x;
